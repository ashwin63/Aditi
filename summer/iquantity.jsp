...
<head><link rel="stylesheet" type="text/css" href="layout.css">
	...
	String page1="0";
	   ...
	   String temporarysize1="",temporaryspeed="",tempadd="",id="",id1="",noproducid="";
	   int teeid=0,sa=0;
		   try{
	    
	    temporarysize1=request.getParameter("size");
        temporaryspeed=request.getParameter("speed");
		noproducid=request.getParameter("noproductid");
	    tempadd="C:";
        id=request.getParameter("id");
        teeid=Integer.parseInt(id);
		
		Scanner temporaryscanner=new Scanner(new File(tempadd+"/tomcat/webapps/summer/WEB-INF/classes/files/pvalues/pdetails.txt"));
        
	    temporaryscanner.nextLine();
        while(teeid!=0)
        {
            line4=temporaryscanner.nextLine();
            teeid--;
        }
        temporaryscanner.close();
        Scanner temporarystringscanner=new Scanner(line4).useDelimiter(",");
        id1=temporarystringscanner.next();
         sa=Integer.parseInt(id1);
		 id=id1;
         ...
         speed=speed1; 
          //assigning correct values to speed variable
		   ...
		   {
			   if(noproducid.equals("0"))
			   {
				   page1="iselect.jsp?size="+temporarysize1+"&speed="+temporaryspeed;
			   }
			   else if (noproducid.equals("1")){
			   page1="index.html";
			   }
		   }
		   catch(Exception e)
		   {
				 //out.println("Please Click Only When the RED Cursor points to your desired option..!");
				 //RequestDispatcher rd=request.getRequestDispatcher("iselect.jsp?size="+temporarysize1+"&speed="+temporaryspeed);  
				 //rd.include(request, response);  
				
			   page1="info.jsp?size="+temporarysize1+"&speed="+temporaryspeed+"&infoid=1";
		   }
		   //check whether the item has already been selected
			 if(page1.equals("0"))
			 {
			Scanner s2=new Scanner(new File(tempadd+"/tomcat/webapps/summer/WEB-INF/classes/files/pvalues/currentitems.txt"));
			while(s2.hasNext())
			{
				String scannerstring=s2.nextLine();
				Scanner stempvalue=new Scanner(scannerstring).useDelimiter(",");
				String sid=stempvalue.next();
				int sid1=Integer.parseInt(sid);
				if(sid1==sa)
				{
				out.println("This item has already been selected.Please Try Again");
				 page1="info.jsp?size="+temporarysize1+"&speed="+temporaryspeed+"&infoid=2";  
...
<script>
var x=0,minvalue,maxvalue,sizetemp,size1,speed=1,tempdigit=-1,mainintervalid,subintervalid,totalmax=14,needtorefresh;
</script>
<script>
var x=1,i;
function abc(){
                          ...
                          if(maintempdigit>=0)
                          {
                             ...
                             maintempdigit=-1;
                               ...
                               var temp=maxvalue-1;
                                ...
                                $("#"+i).attr("style","border-radius: 8px;background-color: #00FF00; color: black;text-align: center; padding : 15px 32px;font-size:"+<%=size%>+";");
                                tempdigit=x;
   ...
   mainfunc();}
    ...
    window.location="currentwritetofile?size=<%=size%>&speed=<%=speed%>&id=<%=id%>&quantity="+tempdigitpresent;
   clearfunc();
   mainfunc();
    }
    else if(tempdigit==13)
    {    
...
<audio autoplay>
    <source src="voicemp3/selectthequantity.mp3" type="audio/mp3" />
</audio>
    <div class="header" style="color: black;"><center><h1><b>Billing Software</b></h1></center>
</div>
 <div class="topnav"><p style="color:white; font-size:<%=size%>;"> <b>SELECT  THE  QUANTITY </p>
     </div>
...
<button id="13" style="border-radius: 8px;background-color : white; color: black;text-align: center; padding : 15px 32px;font-size:<%=size%>;"><b>BACK</button>
<audio id="audio13">
    <source src="voicemp3/back.mp3" type="audio/mp3" />
</audio><br><br>
</td>
</tr>
</table>
